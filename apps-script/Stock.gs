...
/**
 * ออเดอร์ใบเดียวตามเลขที่ — แอปสต๊อกใช้หาใบที่จะคืนของ
 *
 * แอปสต๊อกไม่มีหน้ารายการออเดอร์ จงใจ: มันเป็นแอปของงานคลัง ไม่ใช่งานขาย
 * คนคืนของรู้เลขที่ใบอยู่แล้ว (จากกล่องพัสดุที่ตีกลับ หรือจากชีทนำเข้า Shopee)
 * จึงค้นทีละใบพอ ไม่ต้องดึงรายการทั้งหมดมาให้เลือก
 */
function getOrder(no) {
  requireStaff_();
  no = String(no || '').trim();
  if (!no) return null;
  return findOrder_(no) || null;
}
