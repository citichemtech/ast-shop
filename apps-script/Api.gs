...
function doGet(e) {
  /* มีกุญแจติดมาในลิงก์ = ลูกค้าเปิดหน้าจ่ายเงินของออเดอร์ใบหนึ่ง
     ...
     ตรวจแค่ว่า "มีพารามิเตอร์ p มาไหม" ตรงนี้ ยังไม่ตัดสินว่ากุญแจถูกหรือผิด
     กุญแจผิดต้องได้หน้าลูกค้าที่บอกว่าลิงก์ใช้ไม่ได้ ไม่ใช่ตกไปเจอหน้าล็อกอิน
  ...
  if (hLast >= DATA_ROW) {
    ...
    for (var i = 0; i < hv.length; i++) {
      ...
      var o = {
        ...
        /* เบอร์ลูกค้าที่ชีทเก็บเป็นตัวเลข ศูนย์หน้าจะหายไป 0812345678 กลายเป็น 614035852
...
/**
 * ส่งใบให้ลูกค้าทางอีเมล พร้อมตราประทับบริษัท
 *
 * ทำไมตราอยู่ตรงนี้ที่เดียว: ใบที่พิมพ์ลงกระดาษ ร้านปั๊มตรายางเองอยู่แล้ว
 * แต่ไฟล์ที่ส่งทางอีเมลปั๊มมือไม่ได้ ระบบจึงต้องวาดตราให้เฉพาะฉบับที่ส่งอีเมล
 * ถ้าวาดให้ทุกใบ ใบที่พิมพ์ออกมาจะมีตราสองอัน และอันที่พิมพ์ก็ไม่ใช่ตราจริง
 *
 * หน้าจอเป็นคนวาดกระดาษ (แบบเดียวกับที่ใช้พิมพ์) แล้วส่งรูปมาที่นี่
 * ฝั่งนี้แปลงเป็น PDF แล้วแนบไป — ลูกค้าเปิดได้ทุกเครื่องโดยไม่ต้องมีแอปอะไร
 *
 * ส่งซ้ำได้ ไม่ได้ห้าม (ลูกค้าบอกว่าไม่ได้รับ เป็นเรื่องปกติ)
 * แต่จดไว้ทุกครั้งว่าส่งไปเมื่อไร ถึงใคร จะได้ตอบได้ว่าส่งไปกี่รอบแล้ว
 */
function emailDoc(payload) {
  var email = requireStaff_();
  var p = payload || {};
  var want = String(p.no || '').trim();
  if (!want) throw new Error('ไม่ได้บอกว่าจะส่งใบไหน');

  var to = String(p.to || '').trim();
  if (!to) throw new Error('ใบนี้ไม่มีอีเมลลูกค้า — ใส่อีเมลในใบก่อน แล้วค่อยส่ง');
  /* ที่อยู่อีเมลผิดหนึ่งตัวอักษร = ใบกำกับภาษีหลุดไปหาคนอื่น
     กันแบบหยาบ ๆ ตรงนี้ก่อน ดีกว่าปล่อยให้ MailApp โยน error ที่อ่านไม่รู้เรื่อง */
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) {
  ...
  /* ใบที่ยกเลิกแล้วห้ามหลุดออกจากร้านเด็ดขาด ไม่ว่าทางไหน
     กระดาษมีตราว่ายกเลิกให้เห็น แต่คนกดส่งอีเมลอาจไม่ทันดู */
  var voided = String(cur.voidWhy || '').trim();
  if (voided) {
  ...
  var cfg = appCfg_();
  var co = (cfg.co || {});
  var shop = String(co.name || co.shortName || '').trim() || 'ทางร้าน';
  var subject = String(p.subject || '').trim() ||
    (String(cur.type || 'เอกสาร') + ' เลขที่ ' + want + ' จาก ' + shop);
  /* ตัวสำรองเผื่อหน้าจอไม่ได้ส่งข้อความมา — ปกติหน้าจอส่งมาเสมอ
     เพราะคนกดต้องอ่านและแก้ได้ก่อน ตัวนี้จึงเป็นทางที่ไม่ควรถูกใช้
     แต่ต้องมี ไม่งั้นอีเมลจะออกไปเป็นตัวเปล่า ๆ ถ้าหน้าจอเปลี่ยนไปวันหลัง */
  var body = String(p.body || '').trim() ||
    ('เรียน ' + (String(cur.custName || '').trim() || 'คุณลูกค้า') + '\n\n' +
     'บริษัทฯ ได้ส่งเอกสาร ' + String(cur.type || 'เอกสาร') + ' เลขที่ ' + want +
     ' ยอดรวม ' + money_(cur.total) + ' บาท แนบมาพร้อมอีเมลฉบับนี้\n\n' +
     'ขอแสดงความนับถือ\n' + shop);

  /* ต้องเป็น GmailApp ไม่ใช่ MailApp

     MailApp ส่งผ่านบริการเมลของ Apps Script ซึ่งไม่ผ่านกล่องจดหมายของคนส่ง
     แปลว่าส่งสำเร็จแต่ไม่มีสำเนาอยู่ในโฟลเดอร์ "ส่งแล้ว" เลยสักฉบับ
     ร้านที่ส่งใบกำกับภาษีให้ลูกค้าต้องพิสูจน์ได้ว่าส่งไปแล้วจริง เมื่อไร ถึงใคร
     — ของจริง 18 ก.ย. 69 เจ้าของร้านเปิดช่อง "ส่งแล้ว" แล้วไม่เจออะไรเลย
     GmailApp ส่งผ่าน Gmail ของคนที่กำลังเปิดแอป จึงมีสำเนาเก็บไว้ตามปกติ
  ...
  /* จดในช่องร่องรอยการแก้ใบ ซึ่งเป็นช่องหลังบ้านล้วน ไม่ถูกพิมพ์ลงกระดาษ
     ...
     ซึ่งเป็นงานที่เจ้าของร้านต้องไปทำในชีทเอง และพลาดได้ */
  var at = stampTime_();
  var trail = String(cur.revise || '');
  writeRow_('doc', row, {
    revise: (trail ? trail + ' ' : '') + '[ส่งอีเมล ' + at + ' → ' + to + ']'
  });
  SpreadsheetApp.flush();
 ...
 */
/** ชนิดนี้เป็นใบเสนอราคาไหม — ถามจาก DOC_TYPES ไม่ใช่เทียบชื่อไทยด้วยมือ
    วันที่มีคนแก้ชื่อชนิดเอกสารในชีท การเทียบสตริงตรง ๆ จะเงียบ ๆ ผิดไปเอง */
function isQuoteTh_(th) {
  var t = docTypeByTh_(th);
  return !!(t && t.quote);
}
...
function listDocs(orderNo) {
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    if (want) {
      if (ord !== want) continue;
    } else {
      /* ไม่ระบุออเดอร์ = หน้าใบเสนอราคา ต้องได้ "ใบเสนอราคา" เท่านั้น
         ของเดิมกรองด้วย "ไม่มีเลขออเดอร์" อย่างเดียว ซึ่งไม่ใช่เรื่องเดียวกัน
         ใบเสร็จ/ใบกำกับภาษีที่ออกโดยไม่ผูกออเดอร์ก็ไม่มีเลขออเดอร์เหมือนกัน
         จึงไปกองปนอยู่ในรายการใบเสนอราคา แล้วเจ้าของร้านเปิดมาเห็นใบเสร็จสองใบ
         นึกว่าเอกสารที่เหลือหายไปทั้งหมด ทั้งที่อยู่ในชีทครบ (เจอจริง 15 ก.ย. 69)
         ใบกำกับภาษีต้องอยู่หน้าของใบกำกับภาษี ไม่ใช่มาปนกับใบที่ยังไม่ได้ขาย */
      if (ord) continue;
      if (!isQuoteTh_(th)) continue;
    }
...
function findDocs(p) {
  ...
  /* เรียงตาม "เลขที่เอกสาร" ไม่ใช่ตามลำดับแถวในชีท
     เล่มใบกำกับภาษีต้องเรียงต่อกันไม่ขาด และเป็นสิ่งที่สรรพากรตรวจ
     คนเปิดหน้านี้จึงเปิดมาเพื่อไล่สายตาดูว่าเลขเรียงครบไหม
     ของเดิมเรียงตามแถวในชีท ซึ่งบังเอิญตรงกับลำดับเลขเกือบตลอด
     แต่ไม่เสมอไป (fillDocGaps เติมเลขที่ขาดต่อท้ายแถวสุดท้าย)
     พอไม่ตรงขึ้นมา เลขจะกระโดดสลับไปมาโดยไม่มีเหตุผลที่คนอ่านมองออก

     เทียบส่วนตัวเลขท้ายเลขที่แบบตัวเลขจริง ไม่ใช่เทียบสตริง
     ไม่งั้น ONIV26-00099 จะมาอยู่หลัง ONIV26-00100 เพราะ '9' > '1' */
  function docSeq_(no) {
    var m = String(no || '').match(/(\d+)\s*$/);
    return m ? Number(m[1]) : -1;
  }
  /* เรียงเฉพาะตอนเปิดแฟ้มของชนิดเดียว ซึ่งคือตอนที่คนไล่ดูเล่ม
     ตอนค้นหาข้ามชนิด เลขคนละชุดเทียบกันไม่ได้ (ONIV26-00290 กับ CS26-00003
     ไม่มีใครบอกได้ว่าอันไหนมาก่อน) ปล่อยเรียงตามใบล่าสุดเหมือนเดิมจะตรงกว่า */
  if (type && !want) {
    hit.sort(function (a, b) { return docSeq_(b.no) - docSeq_(a.no); });
  }
...
function getDoc(no) {
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    var m = {
      ...
      /* ชนิดใบเป็นคำไทยที่หน้าจอเอาไปขึ้นหัวข้ออีเมล — ของเดิมส่งกลับแต่ key
         หน้าจอเลยเขียนหัวข้อว่า "เอกสาร เลขที่ ..." แทนชื่อใบจริง */
      type: th,
...
/**
 * ราคาที่คนคีย์พิมพ์มา "รวม VAT แล้ว" — ถอดออกก่อนเขียนลงชีท
 *
 * ชีทเป็นคนคิด VAT เอง ตามสูตร
 *   VAT   = (ยอดสินค้า − ส่วนลด + ค่าจัดส่ง) × อัตรา
 *   สุทธิ = ยอดสินค้า − ส่วนลด + ค่าจัดส่ง + VAT
 * ถ้าเขียนราคาที่รวม VAT แล้วลงไปตรง ๆ ชีทจะบวก 7% ทับเข้าไปอีกชั้น
 * ลูกค้าตกลง 471 แล้วบิลออกมา 504 ซึ่งเป็นเงินที่เก็บไม่ได้
 *
 * ทุกช่องที่อยู่ในฐานภาษีต้องหารด้วย (1+อัตรา) พร้อมกันทั้งชุด
 * ทั้งราคาสินค้า ส่วนลด และค่าจัดส่ง ไม่งั้นสัดส่วนเพี้ยน
 *   สุทธิ = (สินค้า − ส่วนลด + ค่าส่ง)/(1+r) × (1+r) = ยอดที่พิมพ์เข้ามาพอดี
 *
 * ราคาที่เว้นว่างไว้ไม่แตะ เพราะนั่นแปลว่า "ใช้ราคามาตรฐานจากฐานสินค้า"
 * ซึ่งเป็นราคาก่อน VAT อยู่แล้ว ถอดซ้ำจะกลายเป็นขายถูกกว่าที่ตั้งไว้
 * ต้นทุนก็ไม่แตะ เพราะต้นทุนไม่ใช่ราคาที่เก็บจากลูกค้า
 */
function inclToExcl_(p, rate) {
  var d = 1 + Number(rate || 0);
  if (!(d > 1)) return p;
  var cut = function (v) {
    if (v === '' || v === null || v === undefined) return v;
    var n = Number(v);
    return isNaN(n) ? v : round2_(n / d);
  };
  var out = {};
  for (var k in p) out[k] = p[k];
  out.items = (p.items || []).map(function (it) {
    var c = {};
    for (var f in it) c[f] = it[f];
    c.price = cut(it.price);
    return c;
  });
  out.discount = cut(p.discount);
  out.ship = cut(p.ship);
  return out;
}
...
function planOrder_(p, email) {
  ...
  /* "ราคารวม VAT แล้ว" เป็นเรื่องของวิธีอ่านตัวเลขที่พิมพ์เข้ามา ไม่ใช่ของที่เก็บลงชีท
     ถอดตรงนี้ทีเดียวตั้งแต่ต้นทาง ที่เหลือทั้งเส้นทาง (ยอดรวม ตัดล็อต ตรวจยอด)
     จึงทำงานกับตัวเลขก่อน VAT เหมือนเดิมทุกอย่าง ไม่ต้องรู้เรื่องนี้เลย */
  if (String(p.vatMode || '') === 'incl' && p.vat) {
    p = inclToExcl_(p, cfg.vatRate);
  }
