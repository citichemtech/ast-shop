...
function emailDoc(payload) {
  ...
  var body = String(p.body || '').trim() ||
    ('เรียน ' + (String(cur.custName || '').trim() || 'ลูกค้าผู้มีอุปการคุณ') + '\n\n' +
     'แนบ' + String(cur.type || 'เอกสาร') + ' เลขที่ ' + want +
     ' ยอดรวม ' + money_(cur.total) + ' บาท มาพร้อมอีเมลฉบับนี้ค่ะ\n\n' +
     'หากมีข้อสงสัยกรุณาแจ้งกลับได้เลยนะคะ\n\n' +
     'ขอบคุณค่ะ\n' + shop);
