...
var SH = {
  ...
  prod: {
    name: 'ฐานสินค้า',
    /* N O P เติมทีหลังด้วย setupShopColumns() สำหรับหน้าร้านที่ลูกค้าเปิดเอง
       ต่อท้ายช่องสูตร (K L M) ไม่แทรกกลาง ไม่งั้นสูตรของชีทอื่นที่ชี้มาจะเลื่อนหมด
       รูปมีสองช่อง เพราะเครื่องมือตัวเดียวลูกค้าอยากเห็นทั้งตัวสินค้าและปลายคม
       รูปเดียวตอบไม่ได้ว่าของจริงหน้าตาแบบไหน แล้วก็ต้องมาถามทางไลน์อยู่ดี */
    IN: { sku: 2, group: 3, name: 4, perPack: 5, unit: 6, cost: 7, price: 8, opening: 9, reorder: 10,
          web: 14, img: 15, img2: 16, tag: 17 },
    CALC: [1, 11, 12, 13],
    probe: 1
  },
  ...
  head: {
    ...
    IN: {
      ...
      /* ขายผ่านมาร์เก็ตเพลส เงินที่เข้าร้านไม่เท่ากับยอดขาย แพลตฟอร์มหักไปก่อน
         ของจริงที่วัดได้จากไฟล์ช้อปปี้วันเดียว: ขาย 13,182 โดนหัก 2,591 = 19.7%
         บวกค่าส่งที่ร้านออกเองอีก 300 รวมเป็น 21.9% ของยอดขาย
         ถ้าไม่มีสองช่องนี้ กำไรในชีทจะสูงกว่าความจริงราวหนึ่งในห้าทุกใบที่ขายช้อปปี้
         แล้วถ้าเอาตัวเลขนั้นไปตั้งราคาหรือจัดโปร ก็ตัดสินใจบนกำไรที่ไม่มีอยู่จริง */
      fee: 25,       // Y = ค่าธรรมเนียมแพลตฟอร์ม (คอมมิชชั่น + transaction + ค่าบริการ)
      shipCost: 26,  // Z = ค่าส่งที่ร้านออกเอง (ไม่ใช่ค่าส่งที่เก็บจากลูกค้า)
      /* วันเวลาที่กดบันทึกจริง — คนละเรื่องกับช่อง "วันที่" (B) ซึ่งเป็นวันของออเดอร์
         ที่คนคีย์เลือกเอง และย้อนหลังได้ ช่องนี้ระบบเขียนเอง แก้ไม่ได้
         มีไว้ตอบคำถามว่า "ใบนี้เข้ามาตอนกี่โมง" ซึ่งวันที่อย่างเดียวตอบไม่ได้
         ออเดอร์เก่าที่คีย์ก่อนมีช่องนี้จะว่าง — ว่างคือไม่รู้ ไม่ใช่เที่ยงคืน */
      keyedAt: 27    // AA = เวลาที่คีย์เข้าระบบ
    },
    CALC: [10, 13, 14, 15, 16, 18, 21],
    /* จำนวนคอลัมน์ที่ readOrders_ อ่านมาทั้งแถว — ต้องคลุมถึง AA */
    width: 27,
  ...
  doc: {
    ...
    IN: {
      ...
      sentAt: 23,
      /* ร่องรอยการแก้ใบ — ใครแก้ เมื่อไร ยอดเดิมเท่าไร
         แยกออกมาจากช่อง "หมายเหตุ" โดยตั้งใจ เพราะหมายเหตุถูกพิมพ์ลงกระดาษจริง
         ของเดิมจดร่องรอยนี้ไว้ในหมายเหตุ ผลคือทุกใบที่เคยถูกแก้ พอพิมพ์ส่งลูกค้า
         จะมีบรรทัด "[แก้ไขครั้งที่ 1: ... ยอดเดิม ... โดย ...]" ติดไปบนใบด้วย
         ซึ่งเป็นเรื่องหลังบ้านล้วน ๆ ที่ลูกค้าไม่ควรเห็น และคนใช้ต้องมานั่งลบเองทุกใบ
         ช่องนี้ไม่มีอะไรอ่านไปพิมพ์ จึงจดได้เต็มที่โดยไม่หลุดออกไปนอกร้าน */
      revise: 24
    },
    CALC: [1],
    probe: 1
   ...
   */
  /**
   * คำขอสั่งซื้อจากหน้าร้าน — ยังไม่ใช่ออเดอร์
   *
   * แยกจาก ออเดอร์_หัวบิล โดยตั้งใจ เพราะคนที่กรอกมาคือใครก็ได้บนอินเทอร์เน็ต
   * จะให้ของพวกนี้ไปปนกับใบจริงที่ใช้ปิดบัญชีและตัดสต๊อกไม่ได้
   * พนักงานตรวจแล้วกด "รับเป็นออเดอร์" ถึงจะข้ามฝั่งไปเป็นใบจริง
   *
   * ช่อง lines เก็บแบบ SKU*จำนวน คั่นด้วย · — คนอ่านออก และโค้ดแกะกลับได้
   * ไม่ต้องมีชีทลูกอีกใบสำหรับรายการสินค้าของคำขอที่อาจไม่ได้กลายเป็นออเดอร์เลย
   */
  req: {
    name: 'คำขอสั่งซื้อ',
    IN: {
      no: 2, at: 3, cust: 4, tel: 5, addr: 6, note: 7,
      lines: 8, names: 9, est: 10, status: 11, orderNo: 12, by: 13, why: 14
    },
    CALC: [1],
    probe: 1
  },
  ...
  /**
   * แบนเนอร์บนหน้าร้าน — รูปใหญ่ที่ลูกค้าเห็นก่อนสินค้า
   *
   * อยู่ในชีท ไม่ได้ฝังในโค้ด เพราะแบนเนอร์คือของที่เปลี่ยนบ่อยที่สุดในร้าน
   * โปรเดือนนี้จบ เดือนหน้าเปลี่ยน ถ้าฝังในโค้ดต้องรอคนแก้โค้ดให้ทุกเดือน
   *
   * ช่อง "ตำแหน่ง" บอกว่าแบนเนอร์ไปโผล่แถบไหนของหน้าแรก
   * ช่อง "ลิงก์ปุ่ม" รับได้สามแบบ: ที่อยู่เว็บเต็ม ๆ · หมวด:<ชื่อหมวด> · สินค้า:<SKU>
   * สองแบบหลังทำให้กดแล้ววิ่งไปหน้าในร้านเอง ไม่ต้องเด้งออกไปข้างนอก
   */
  ban: {
    name: 'แบนเนอร์หน้าร้าน',
    IN: { slot: 2, title: 3, img: 4, btn: 5, href: 6, on: 7, note: 8 },
    CALC: [1],
    probe: 1
  },

  /**
   * หมวดสินค้าบนหน้าร้าน — รูปไอคอนกับรูปปกของแต่ละหมวด
   *
   * ชื่อหมวดยังมาจากคอลัมน์ C ของ ฐานสินค้า เหมือนเดิม ชีทนี้ไม่ได้สร้างหมวดใหม่
   * มีไว้แต่งหน้าตาเท่านั้น — หมวดที่ไม่มีแถวในนี้ก็ยังขึ้นหน้าร้านตามปกติ
   * แค่ไม่มีรูป จะได้ไม่มีวันที่เพิ่มหมวดในฐานสินค้าแล้วสินค้าหายไปจากหน้าร้าน
   */
  scat: {
    name: 'หมวดหน้าร้าน',
    IN: { group: 2, label: 3, icon: 4, cover: 5, home: 6, note: 7 },
    CALC: [1],
    probe: 1
  },
 ...
 *
 * เบอร์ 0812345678 ถูกชีทอ่านเป็นตัวเลขแล้วศูนย์หน้าหาย
 * และเบอร์ที่มีขีดคั่นอย่าง 081-234-5678 ยิ่งหนักกว่า เพราะกลายเป็นการลบเลข
...
var TEXT_IN = {
  ...
  link: { key: 1 },
  /* เบอร์ลูกค้าโดนเรื่องศูนย์หน้าหายเหมือนกันทุกที่ที่เก็บเบอร์ */
  req: { tel: 1, lines: 1 }
};
...
function appCfg_() {
  ...
  var out = {
    ...
    shipFee: 0, freeOver: 0, codFee: 0, codMax: 2000, line: '',
    ...
    shopOpen: true,
    shopMode: 'queue',
    /* หน้าตาหน้าร้าน — โลโก้ ภาพหัว และลิงก์แผนที่ เว้นว่างได้หมด
       ว่างแล้วหน้าร้านใช้พื้นหลังไล่สีกับชื่อร้านเป็นตัวหนังสือแทน ไม่พัง */
    shopLogo: '',
    shopCover: '',
    shopMap: ''
  };
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    /* วงเงินปลายทาง: ยอดเกินกว่านี้ไม่ให้ลูกค้าเลือกเก็บเงินปลายทาง
       ช่องว่างในชีท = ยังไม่เคยตั้ง ใช้ค่าเริ่มต้น 2000 ตามที่เจ้าของร้านกำหนด
       ส่วน 0 ที่พิมพ์ลงไปเอง = ปิดปลายทางทั้งร้าน คนละความหมายกับช่องว่าง
       ถ้าไม่แยกสองอย่างนี้ เจ้าของร้านจะปิดปลายทางไม่ได้เลย เพราะพิมพ์ 0 แล้วเด้งกลับเป็น 2000 */
    else if (k === 'วงเงินสูงสุดเก็บเงินปลายทาง') {
      out.codMax = String(val === 0 ? '0' : (val || '')).trim() === '' ? 2000 : (Number(val) || 0);
    }
    else if (k === 'ลิงก์ LINE ของร้าน') out.line = String(val || '');
    else if (k === 'โลโก้ร้าน (ลิงก์รูป)') out.shopLogo = String(val || '');
    else if (k === 'ภาพหัวหน้าร้าน (ลิงก์รูป)') out.shopCover = String(val || '');
    else if (k === 'ลิงก์แผนที่ร้าน') out.shopMap = String(val || '');
    else if (k === 'เปิดรับออเดอร์หน้าเว็บ') out.shopOpen = !shopHidden_(val);
    /* "เข้าคิวก่อน" = ไปนอนรอในชีท คำขอสั่งซื้อ ให้พนักงานตรวจแล้วกดรับ
       "เข้าชีทเลย" = เป็นออเดอร์จริงทันที ตัดสต๊อกทันที
       ค่าเริ่มต้นคือเข้าคิวก่อน เพราะพลาดแล้วแก้ง่ายกว่าอีกทางมาก */
    else if (k === 'ออเดอร์จากเว็บ') out.shopMode =
      (String(val || '').indexOf('เลย') > -1 || String(val || '').indexOf('ทันที') > -1)
        ? 'direct' : 'queue';
