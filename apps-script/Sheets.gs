...
var SH = {
  ...
  /**
   * ตารางจับคู่ระหว่างรหัส/ชื่อสินค้าฝั่ง Shopee กับ SKU ในระบบ
   *
   * ต้องมีตารางนี้เพราะไฟล์ที่ Shopee ส่งออกมาไม่มี SKU ของร้านอยู่เลย
   * มีแต่รหัสที่ร้านพิมพ์ไว้ในหน้าสินค้าของ Shopee ซึ่งมักไม่ตรงกับ SKU ในชีท
   * เดาเอาเองไม่ได้ — เดาผิดคือตัดสต๊อกผิดตัว แล้วรู้ตัวอีกทีตอนของขาด
   *
   * ตัวคูณ (F) ไว้สำหรับสินค้าจัดเซต เช่น Shopee ขาย "แพ็ค 5 ชิ้น" 1 ชิ้น
   * ต้องตัดสต๊อกในระบบ 5 ชิ้น ใส่ 5 ที่ช่องนี้
   */
  map: {
    name: 'จับคู่SKU',
    IN: { code: 2, name: 3, variant: 4, sku: 5, mult: 6, note: 9 },
    CALC: [1, 7, 8],
    probe: 1
  },

  /**
   * ทะเบียนออเดอร์ Shopee ที่นำเข้าไปแล้ว 1 ใบ = 1 แถว
   *
   * มีไว้เพื่ออย่างเดียว: กันตัดสต๊อกซ้ำ
   * ไฟล์ที่ export จาก Shopee ช่วงวันซ้อนกันเป็นเรื่องปกติมาก คนกดนำเข้าไฟล์
   * เดือนนี้ทับไฟล์สัปดาห์ที่แล้วเมื่อไร ถ้าไม่มีทะเบียนนี้สต๊อกจะถูกตัดสองรอบ
   * และไม่มีอะไรฟ้อง
   */
  imp: {
    name: 'นำเข้า Shopee',
    IN: { sn: 2, date: 3, status: 4, orderNo: 5, amount: 6, fee: 7, state: 8, at: 9, by: 10, note: 11 },
    CALC: [1],
    probe: 1
  },
