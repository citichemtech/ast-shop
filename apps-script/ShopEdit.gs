...
function saveShopLook(v) {
  ...
  if (v.payLink !== undefined) {
    ...
    if (v.payLink) {
      ...
      if (mine && v.payLink.split('?')[0] === mine.split('?')[0]) {
        throw new Error('อันนี้เป็นลิงก์ของพนักงาน (ลิงก์เดียวกับที่คุณเปิดอยู่ตอนนี้) ' +
          'ลูกค้ากดแล้วจะเจอหน้าให้ล็อกอิน Google\n\n' +
          'ต้องใช้ลิงก์ของ deploy อีกตัวที่ตั้ง "ทำงานในชื่อ: ฉัน" และ ' +
          '"ใครเข้าถึงได้: ทุกคน" — ใน Apps Script กด Deploy → การทำให้ใช้งานได้ใหม่');
      }
