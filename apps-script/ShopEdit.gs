...
function shopLinks_(c) {
  ...
  return {
    ...
    why: base ? '' : 'ยังไม่ได้กรอก "ลิงก์เว็บแอปสำหรับลูกค้า" — ลิงก์นี้คือตัวที่ส่งให้ลูกค้า ' +
      'และเป็นฐานของลิงก์จ่ายเงินรายออเดอร์ด้วย\n' +
      'ถ้า deploy ที่ใช้อยู่ตั้งเป็น "ใครเข้าถึงได้: ทุกคน" แล้ว ใช้ลิงก์เดียวกันนี้ได้เลย ' +
      'ถ้าไม่ใช่ ต้องสร้าง deploy อีกตัวที่ตั้ง "ทำงานในชื่อ: ฉัน" และ "ใครเข้าถึงได้: ทุกคน"'
  };
}
...
function saveShopLook(v) {
  ...
  var WANT = [
    ...
    /* กรอกในแอปได้ ไม่ต้องเปิดชีทไปหาแถว — เจ้าของร้านสร้าง deploy สาธารณะเสร็จ
       แล้วมีลิงก์อยู่ในมือทันที ถ้าต้องไปเปิดชีทหาแถวต่ออีกขั้น มันค้างอยู่ตรงนั้น */
    ['ลิงก์เว็บแอปสำหรับลูกค้า', 'payLink', 0,
      'ลิงก์ของ deploy ที่ตั้ง "ทำงานในชื่อ: ฉัน" และ "ใครเข้าถึงได้: ทุกคน" ' +
      'ใช้เป็นฐานของทั้งหน้าร้านและหน้าจ่ายเงิน — คนละตัวกับลิงก์ของพนักงาน']
  ];

  /* ลิงก์ที่ก๊อปมาจากเบราว์เซอร์มักติด ?shop=1 หรือ ?p=... มาด้วย
     ค่านี้ถูกใช้เป็น "ฐาน" ที่ระบบเอาไปต่อท้ายเอง ถ้าเก็บของเดิมไว้จะได้ลิงก์
     ที่มีพารามิเตอร์ซ้อนกันสองชั้น ตัดให้เหลือแค่ถึง /exec ตั้งแต่ตอนบันทึก */
  if (v.payLink !== undefined) {
    v.payLink = String(v.payLink || '').trim().split('#')[0].split('?')[0];
    if (v.payLink) {
      var mine = '';
      try { mine = String(ScriptApp.getService().getUrl() || ''); } catch (e) {}
      /* ลิงก์พนักงานกับลิงก์ลูกค้าหน้าตาเหมือนกันมาก ต่างกันแค่รหัสกลางลิงก์
         วางผิดตัวแล้วลูกค้าจะเจอหน้าให้ล็อกอิน Google ซึ่งดูเหมือนร้านพัง
         และกว่าจะรู้ก็ต่อเมื่อลูกค้าทักมาบ่น — จับให้ได้ตั้งแต่ตอนวาง */
      /* เตือน ไม่ใช่ห้าม

         ตอนแรกทำเป็นห้ามเด็ดขาด เพราะคิดว่ามี deploy สองตัวเสมอ
         แต่ deploy ตัวเดียวที่ตั้ง "ทำงานในชื่อ: ฉัน" + "ใครเข้าถึงได้: ทุกคน"
         ใช้เป็นทั้งหน้าพนักงานและหน้าลูกค้าได้จริง ถ้าเจ้าของร้านเปิดเอง
         (เจ้าของสคริปต์ยังได้อีเมลตัวเองกลับมา ด่าน requireStaff_ จึงยังผ่าน)
         ห้ามเด็ดขาดคือบังคับให้เขาไปสร้าง deploy ที่ไม่จำเป็นอีกตัว แล้วก็ค้างอยู่ตรงนั้น

         แต่ถ้า deploy ตั้งเป็น "ผู้ใช้ที่มีบัญชี Google" ลูกค้าจะเจอหน้าล็อกอินจริง
         และไม่มีใครมาบอก เพราะลูกค้าปิดทิ้งไปเฉย ๆ — ความเสี่ยงนี้ใหญ่พอที่จะเตือน
         ทางออกคือให้ยืนยันด้วยตัวเองหลังจากไปทดสอบมาแล้ว ไม่ใช่กดผ่านโดยไม่รู้ตัว */
      if (mine && v.payLink.split('?')[0] === mine.split('?')[0] && !v.payLinkSure) {
        throw new Error('SAME_AS_STAFF|อันนี้เป็นลิงก์เดียวกับที่คุณเปิดอยู่ตอนนี้\n\n' +
          'ถ้า deploy ตัวนี้ตั้งเป็น "ใครเข้าถึงได้: ทุกคน" ก็ใช้ได้เลย ' +
          'แต่ถ้าตั้งเป็น "ผู้ใช้ที่มีบัญชี Google" ลูกค้าจะเจอหน้าให้ล็อกอิน\n\n' +
          'ทดสอบก่อน: เปิดหน้าต่างไม่ระบุตัวตนแล้ววางลิงก์นี้ต่อท้ายด้วย ?shop=1\n' +
          '  เห็นหน้าร้าน = ใช้ได้ กดยืนยันได้เลย\n' +
          '  เจอหน้าล็อกอิน = ต้องแก้ค่า deploy ก่อน (Deploy → การทำให้ใช้งานได้ใหม่)');
      }
      if (!/\/exec$/.test(v.payLink)) {
        throw new Error('ลิงก์เว็บแอปต้องลงท้ายด้วย /exec — ที่วางมาคือ ' + v.payLink +
          '\n\nถ้าลงท้ายด้วย /dev อันนั้นเป็นลิงก์ทดสอบ เปิดได้เฉพาะคุณคนเดียว');
      }
    }
  }
