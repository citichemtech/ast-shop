...
/* ============================== 8. ต้องไม่ทำให้หน้าจอช้าลง */
console.log('\n8. เปิดหน้าเก็บเงินหนึ่งครั้ง ต้องไม่ลากทั้งชีทมาทั้งสามพันแถว');
/* เจอกับของจริง 13 ก.ย. 69: เจ้าของร้านบอกว่า "ช้ามากกกก" หลังเพิ่มชีทลิงก์
   ต้นเหตุคือทุกการอ่านชีทอ่านถึงแถวสุดท้ายที่มีสูตร = สามพันแถวเสมอ
   ต่อให้มีของจริงไม่กี่แถว และทุกชีทที่เพิ่มเข้ามาก็จ่ายค่านี้เท่ากันหมด

   วัดเป็น "จำนวนช่องที่อ่านออกจากชีท" เพราะของจริงคิดค่าตรงนี้เป็นเวลารอ
   ไม่ได้จับเวลา ซึ่งบนเครื่องทดสอบเร็วจนไม่เห็นความต่าง */
var s8 = start();
var no8 = s8.order({ vat: true });
s8.ctx.payLink(no8);
FS.CELLS_READ.n = 0;
s8.ctx.payAsk(no8);
var cells8 = FS.CELLS_READ.n;
console.log('   เปิดหน้าเก็บเงินหนึ่งครั้ง อ่านไป ' + cells8 + ' ช่อง');
/* ชีทสลิปกับชีทลิงก์อย่างละสามพันแถว ถ้ายังอ่านแบบเดิมจะเกินห้าหมื่นช่องแน่ ๆ
   เลขเพดานตั้งไว้หลวม ๆ เพื่อจับ "กลับไปอ่านทั้งชีท" ไม่ใช่ไล่บี้ทุกช่อง */
truthy('อ่านน้อยกว่าสองหมื่นช่อง', cells8 < 20000);

FS.CELLS_READ.n = 0;
s8.ctx.pubOrder(s8.ctx.payLink(no8).key);
var cells8b = FS.CELLS_READ.n;
console.log('   ลูกค้าเปิดลิงก์หนึ่งครั้ง อ่านไป ' + cells8b + ' ช่อง');
truthy('ฝั่งลูกค้าก็ต้องไม่ลากทั้งชีทมา', cells8b < 20000);

/* ================================================== 9. เก็บเงินปลายทาง */
console.log('\n9. ใบเก็บเงินปลายทาง ต้องไม่มีทางให้ลูกค้าโอนซ้ำ');
/* เจ้าของร้านสั่งเมื่อ 14 ก.ย. 69: ให้มีสถานะเก็บเงินปลายทาง และยืนยันว่า
   "ถ้าลูกค้าเลือกปลายทางก็ 50 ไม่ต้องเพิ่มจัดส่ง" — ค่าส่งเท่าเดิม ไม่บวกค่าบริการ

   อันตรายจริงของใบปลายทางไม่ใช่ตัวเลข แต่คือลูกค้าจ่ายสองรอบ:
   โอนตามลิงก์ที่ร้านส่งไป แล้วยังจ่ายพนักงานส่งของอีกครั้งตอนรับของ
   เงินก้อนที่สองนั้นร้านต้องตามคืน ซึ่งเสียทั้งเงินและความเชื่อใจ
   ทางเดียวที่กันได้จริงคือไม่ให้มีปุ่มโอนอยู่ตรงนั้นตั้งแต่แรก */
var s9 = start();
var no9 = s9.order({ vat: true, status: 'เก็บเงินปลายทาง' });

console.log('\n   ค่าส่งต้องเท่าเดิม ระบบไม่แอบบวกค่าบริการปลายทาง');
var ord9 = s9.ctx.getOrders(0).filter(function (o) { return o.no === no9 })[0];
eq('สถานะลงชีทตามที่เลือก', ord9.status, 'เก็บเงินปลายทาง');
eq('ค่าจัดส่งยังเป็น 50 เท่าใบธรรมดา', ord9.ship, 50);
var plain9 = s9.order({ vat: true });
var ordP9 = s9.ctx.getOrders(0).filter(function (o) { return o.no === plain9 })[0];
eq('ยอดสุทธิเท่ากับใบเดียวกันที่ไม่ใช่ปลายทาง', ord9.net, ordP9.net);

console.log('\n   หน้าเก็บเงินฝั่งพนักงาน');
var A9 = s9.ctx.payAsk(no9);
eq('บอกหน้าจอว่าใบนี้ปลายทาง', A9.cod, true);
eq('ไม่ทำ QR ให้', A9.qr, null);
eq('และไม่ใช่ความผิดพลาด แค่ไม่ต้องใช้', A9.qrOff, true);
truthy('บอกเหตุผลเป็นภาษาคน', A9.qrWhy.indexOf('ปลายทาง') > -1);
eq('ไม่มีข้อความชวนโอนให้ก๊อป', A9.msg, '');
eq('ส่งชื่อขนส่งมาให้ เผื่อกรอกตอนขนส่งโอนยอดกลับ', A9.carrier, 'Flash Express');

console.log('\n   ปุ่มสร้างลิงก์ให้ลูกค้าจ่ายเอง ต้องกดไม่ได้');
var m9 = throws('ปฏิเสธการสร้างลิงก์', function () { s9.ctx.payLink(no9); }, 'ปลายทาง');
truthy('และบอกทางออกไว้ด้วย ไม่ใช่ปฏิเสธเฉย ๆ', m9.indexOf('รอชำระ') > -1);
truthy('บอกด้วยว่าทำไม — จ่ายสองรอบ', m9.indexOf('สองรอบ') > -1);

console.log('\n   ลิงก์ที่สร้างไว้ "ก่อน" เปลี่ยนเป็นปลายทาง คือใบที่อันตรายที่สุด');
/* กันปุ่มอย่างเดียวไม่พอ ลิงก์ที่ส่งไปเมื่อวานยังเปิดได้อยู่จริง
   ลูกค้าที่เก็บลิงก์ไว้ในแชทแล้วกดวันนี้ ต้องเจอหน้าที่ไม่มีเลขบัญชี */
var s9b = start();
var no9b = s9b.order({ vat: true });
var K9b = s9b.ctx.payLink(no9b).key;
eq('ตอนเป็นใบปกติ เห็นเลขบัญชีตามปกติ', s9b.ctx.pubOrder(K9b).acct.acct, '431-039435-5');
s9b.ctx.setTracking(no9b, '', 'เก็บเงินปลายทาง');
var G9b = s9b.ctx.pubOrder(K9b);
eq('พอเปลี่ยนเป็นปลายทาง หน้าลูกค้าบอกว่าปลายทาง', G9b.cod, true);
eq('และไม่มีเลขบัญชีให้โอนอีกต่อไป', G9b.acct, null);
eq('ใบยังไม่ตาย ลูกค้ายังดูรายการกับยอดได้', G9b.dead, false);
eq('ยังเห็นยอดที่ต้องเตรียมให้พนักงานส่งของ', G9b.ord.net > 0, true);
throws('และแนบสลิปไม่ได้ เพราะไม่ควรมีสลิปให้แนบ',
  function () { s9b.ctx.pubSlip(K9b, { data: PNG1 }); }, 'ปลายทาง');

console.log('\n   เปลี่ยนใจกลับมาโอน ต้องกลับมาใช้ได้เหมือนเดิม');
s9b.ctx.setTracking(no9b, '', 'รอชำระ');
eq('เลขบัญชีกลับมา', s9b.ctx.pubOrder(K9b).acct.acct, '431-039435-5');
eq('สร้างลิงก์ได้อีกครั้ง และเป็นกุญแจเดิม', s9b.ctx.payLink(no9b).key, K9b);
eq('แนบสลิปได้แล้ว', s9b.ctx.pubSlip(K9b, { data: PNG1 }).ok, true);

eq('ชุดปลายทางไม่มีช่องสูตรถูกเขียนทับ', hurt(s9.fx).concat(hurt(s9b.fx)), []);
