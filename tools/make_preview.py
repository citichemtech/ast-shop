...
def build_one(src, out):
    ...
    # สองหน้าประกอบไฟล์ไม่เท่ากัน จำนวนที่คาดจึงผูกกับชื่อหน้า ไม่ใช่เลขตัวเดียวทั้งสคริปต์
    # 0 = รวมไฟล์มาแล้ว (bundle.py) ไม่มี include เหลือให้แทน
    want = {"Index.html": 10, "StockIndex.html": 4}.get(src.name)
    if want is not None and n not in (0, want):
        sys.exit("%s คาดว่าจะมี include %d อัน (หรือ 0 ถ้ารวมไฟล์มาแล้ว) แต่เจอ %d อัน"
                 % (src.name, want, n))
