...
/* ---- ถอด VAT ต้องคิดทีละบรรทัด ให้ตรงกับที่ลูกค้าคิดเอง ---- */
console.log('\nถอด VAT แบบรวมราคาแล้ว ต้องถอดทีละบรรทัด');
/* ของจริง 17 ก.ย. 69 ใบ ONIV26-00300 ถูกลูกค้าส่งกลับมาสองรอบเพราะสตางค์เดียว
   ลูกค้าคิดจากราคาที่จ่ายต่อบรรทัด: 480 ÷ 1.07 = 448.60 · 91 ÷ 1.07 = 85.05
   รวมฐาน 533.65 ภาษี 37.35  ส่วนการถอดจากยอดรวมทีเดียวได้ 533.64 / 37.36 */
var dI = ctx.buildDoc_('rec',
  { items: [{ name: 'IPA 1000 ml', qty: 5, price: 96 }], ship: 91 },
  { vatRate: 0.07, vatMode: 'incl' });
eq('ฐานภาษีเท่ากับที่ลูกค้าคิด', dI.base, 533.65);
eq('ภาษีเท่ากับที่ลูกค้าคิด', dI.vat, 37.35);
eq('ยอดรวมเท่ากับที่ลูกค้าจ่ายเป๊ะ', dI.total, 571);
eq('ฐาน + ภาษี = ยอดรวม', Math.round((dI.base + dI.vat) * 100) / 100, dI.total);
eq('บรรทัดสินค้าโชว์ราคาที่จ่ายจริง', dI.lines[0].amount, 480);
eq('ค่าจัดส่งแยกบรรทัด', dI.lines[1].amount, 91);

/* บรรทัดเดียวต้องได้เท่าเดิมกับการถอดจากยอดรวม ไม่มีอะไรเปลี่ยน */
var dOne = ctx.buildDoc_('rec', { items: [{ name: 'ของชิ้นเดียว', qty: 1, price: 149 }] },
  { vatRate: 0.07, vatMode: 'incl' });
eq('ใบบรรทัดเดียว ฐานยังเป็น 139.25 เหมือนเดิม', dOne.base, 139.25);
eq('ใบบรรทัดเดียว ภาษี 9.75 เหมือนเดิม', dOne.vat, 9.75);
