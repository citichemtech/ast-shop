...
eq('ที่อยู่ขึ้นต้นด้วยชื่อลูกค้า แล้วต่อตำบล อำเภอ จังหวัด รหัสไปรษณีย์ ให้ครบ',
  r1.orders[0].addr,
...
eq('ที่อยู่ไม่ซ้ำสามรอบ', r2.orders[0].addr, 'ลูกค้า X\n1 ถ.หนึ่ง');
eq('เบอร์โทรครั้งเดียว', r2.orders[0].tel, '0810000001');
...
console.log('\n4.5 ชื่อที่ Shopee ปิดบังไว้ ต้องยังตามกลับไปหาที่อยู่ได้');
var r45 = ctx.shopeeRead(tsv([HEAD,
  line({ sn: 'M1', name: 'ก', sku: 'SKU-141', price: '100', qty: '1',
         cust: 'D*******n', addr: '9/9 ถ.ทดสอบ', province: 'ชลบุรี', zip: '20000' })]));
eq('ชื่อที่ถูกปิดบังยังถูกเก็บไว้ตามเดิม ไม่ได้ลบทิ้ง', r45.orders[0].cust, 'D*******n');
eq('และไปอยู่บรรทัดแรกของที่อยู่ด้วย ใบปะหน้าจึงมีชื่อผู้รับ',
  r45.orders[0].addr, 'D*******n\n9/9 ถ.ทดสอบ ชลบุรี 20000');

console.log('\n   ชื่อที่มีอยู่ในที่อยู่แล้ว ต้องไม่ใส่ซ้ำสองรอบ');
var r45b = ctx.shopeeRead(tsv([HEAD,
  line({ sn: 'M2', name: 'ก', sku: 'SKU-141', price: '100', qty: '1',
         cust: 'สมชาย', addr: 'สมชาย 9/9 ถ.ทดสอบ' })]));
eq('ไม่ซ้ำ', r45b.orders[0].addr, 'สมชาย 9/9 ถ.ทดสอบ');

console.log('\n   ไม่มีที่อยู่เลย ก็ไม่ต้องสร้างที่อยู่ปลอมจากชื่อ');
var r45c = ctx.shopeeRead(tsv([HEAD,
  line({ sn: 'M3', name: 'ก', sku: 'SKU-141', price: '100', qty: '1', cust: 'สมชาย' })]));
eq('ที่อยู่ยังว่าง', r45c.orders[0].addr, '');
...
eq('ที่อยู่ที่มีขึ้นบรรทัดใหม่ยังอยู่ครบใบเดียว (มีชื่อผู้รับนำหน้า)', r5.orders[0].addr,
  'ทดสอบ, ระบบ\n9/9 ถ.หนึ่ง, ซอยสอง\nอาคารสาม ชั้น 4');
...
eq('ชื่อผู้รับกับที่อยู่อยู่ด้วยกัน', r9.orders[0].addr, 'Test Buyer\n1 Test Road');
eq('เบอร์โทร', r9.orders[0].tel, '0811111111');
