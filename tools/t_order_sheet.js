...
/* ====== 53. สรุปยอดรายเดือน — ยอดขาย ต้นทุน ค่าแอด กำไรจริง

   เจ้าของร้านอยากตอบให้ได้ว่าเดือนนี้เหลือกำไรเท่าไรหลังหักค่าแอด
   ค่าแอดไม่เคยผ่านระบบออเดอร์ ต้องกรอกเอง · เดือนก่อนเริ่มใช้ระบบก็ไม่มีออเดอร์
   ให้คิด ต้องกรอกยอดจากไฟล์เดิม สองแบบนี้อยู่ในตารางเดียวกัน                */
console.log('\n53. สรุปยอดรายเดือน');
...
api53.createOrder(order({ date: '2026-09-02', items: [{ sku: 'SKU-141', qty: 2, price: 500 }] }));
var o53b = api53.createOrder(order({ date: '2026-09-05', items: [{ sku: 'SKU-141', qty: 1, price: 300 }] }));
/* ใบที่ยกเลิกต้องไม่นับเป็นยอดขายของเดือน */
var o53x = api53.createOrder(order({ date: '2026-09-06', items: [{ sku: 'SKU-141', qty: 5, price: 900 }] }));
...
console.log('\n   เดือนที่มีออเดอร์ในระบบ คิดยอดให้เองไม่ต้องกรอก');
var sep53 = m53('2026-09');
truthy2('มีเดือน 2026-09 ในตาราง', !!sep53);
eq('นับเฉพาะใบที่ยังไม่ถูกยกเลิก', sep53.orders, 2);
/* 500*2 + ค่าส่ง 50 = 1050 · 300 + 50 = 350 */
eq('ยอดขายรวมของเดือน', sep53.sales, 1400);
eq('ยังไม่ได้กรอกค่าแอด จึงเป็นศูนย์', sep53.ads, 0);
eq('กำไรสุทธิเท่ากับกำไรขั้นต้นเมื่อยังไม่มีค่าแอด', sep53.net, sep53.gross);
eq('ยอดขายมาจากระบบ ไม่ใช่ที่กรอกเอง', sep53.typedSales, false);

console.log('\n   กรอกค่าแอดแล้วกำไรสุทธิต้องลดลงตาม');
api53.saveMonth({ ym: '2026-09', ads: 400, by: 'เอ๋', clientKey: 'mn53a' });
var sep53b = m53('2026-09');
eq('ค่าแอดเข้าไปแล้ว', sep53b.ads, 400);
eq('กำไรขั้นต้นไม่เปลี่ยน ค่าแอดไม่ใช่ต้นทุนสินค้า', sep53b.gross, sep53.gross);
eq('กำไรสุทธิถูกหักค่าแอด', sep53b.net, sep53.gross - 400);
eq('ยอดขายยังคิดจากระบบเหมือนเดิม', sep53b.sales, 1400);

console.log('\n   เดือนก่อนเริ่มใช้ระบบ ไม่มีออเดอร์ให้คิด ต้องกรอกเอง');
api53.saveMonth({ ym: '2026-01', sales: 120000, cost: 70000, ads: 9000,
                  note: 'จากไฟล์ Excel เดิม', by: 'เอ๋', clientKey: 'mn53b' });
var jan53 = m53('2026-01');
eq('ยอดขายที่กรอกเอง', jan53.sales, 120000);
eq('ต้นทุนที่กรอกเอง', jan53.cost, 70000);
eq('กำไรสุทธิหลังหักค่าแอด', jan53.net, 120000 - 70000 - 9000);
eq('ไม่มีออเดอร์ในระบบสักใบ', jan53.orders, 0);
eq('บอกได้ว่าเลขนี้กรอกมาเอง ไม่ใช่ระบบคิด', [jan53.typedSales, jan53.typedCost], [true, true]);
eq('หมายเหตุถูกเก็บไว้ด้วย', jan53.note, 'จากไฟล์ Excel เดิม');

console.log('\n   ตัวเลขที่กรอกเองต้องชนะตัวเลขที่ระบบคิดได้');
/* เดือนที่ปิดบัญชีไปแล้วต้องล็อกตัวเลขไว้ได้ ไม่ใช่เปลี่ยนไปมาทุกครั้งที่มีคนแก้ใบเก่า */
api53.saveMonth({ ym: '2026-09', sales: 2000, by: 'เอ๋', clientKey: 'mn53c' });
var sep53c = m53('2026-09');
eq('ใช้ยอดที่กรอกเอง', sep53c.sales, 2000);
eq('และบอกว่ามาจากการกรอกเอง', sep53c.typedSales, true);
eq('ต้นทุนที่ไม่ได้กรอก ยังคิดจากระบบ', sep53c.typedCost, false);

console.log('\n   ล้างช่องที่กรอกไว้ ต้องกลับไปใช้ตัวเลขของระบบ');
api53.saveMonth({ ym: '2026-09', sales: '', by: 'เอ๋', clientKey: 'mn53d' });
var sep53d = m53('2026-09');
eq('กลับมาเป็นยอดจากระบบ', sep53d.sales, 1400);
eq('และไม่ใช่เลขที่กรอกเองแล้ว', sep53d.typedSales, false);
eq('ค่าแอดที่ไม่ได้ส่งมาด้วย ต้องไม่หายไปเอง', sep53d.ads, 400);

console.log('\n   เดือนเดิมต้องไม่กลายเป็นสองแถว');
