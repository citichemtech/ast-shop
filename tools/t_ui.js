...
(async function () {
  ...
  /* ขายผ่านมาร์เก็ตเพลส ร้านไม่ได้เก็บค่าส่ง แพลตฟอร์มคิดกับลูกค้าของเขาเอง
     ช่องค่าส่งเติมค่ามาตรฐานของร้านให้เองทุกใบ ถ้าคนคีย์ไม่ทันลบ ใบนั้นจะมี
     ค่าส่ง 50 งอกขึ้นมาทั้งที่ไม่มีใครจ่าย ยอดในชีทเกินจริง และถ้าเป็นใบถอด VAT
     ยอดสุทธิก็จะไม่ตรงกับยอดที่ลูกค้าจ่ายจริงบนแพลตฟอร์ม */
  console.log('\n   ออเดอร์มาร์เก็ตเพลส — ต้องไม่แนะนำค่าส่งของร้าน');
  eq('ปกติแนะนำค่าส่งมาตรฐานของร้าน', await page.inputValue('#f-ship'), '50');
  await page.selectOption('#f-channel', 'Shopee');
  await page.waitForTimeout(120);
  eq('เลือก Shopee แล้วค่าส่งแนะนำเป็น 0', await page.inputValue('#f-ship'), '0');
  await page.selectOption('#f-channel', 'หน้าร้าน');
  await page.waitForTimeout(120);
  eq('กลับมาขายหน้าร้าน ค่าส่งแนะนำกลับมา', await page.inputValue('#f-ship'), '50');
  ...
  /* ค่าตั้งต้นของใบใหม่ต้องเป็นแบบที่ร้านขายจริง (ตัวแรกใน ตั้งค่า!VAT = "ไม่รับ VAT")
     ตอนเพิ่มตัวเลือก "ถอด VAT" ผมเผลอให้ <select> ตกไปที่ตัวเลือกแรกของรายการ
     คือ "บวก VAT 7%" ทุกใบใหม่จึงโดนบวก 7% เงียบ ๆ ทั้งที่ไม่มีใครสั่ง
     ข้อสอบข้อนี้ตรึงค่าตั้งต้นไว้ ไม่ให้พลาดซ้ำแบบเดิมโดยไม่มีใครรู้ */
  eq('ใบใหม่ตั้งต้นเป็นไม่คิด VAT ตามที่ชีทบอก', await page.inputValue('#f-vat'), 'none');
  eq('จึงยังไม่มี VAT ในยอด', await page.textContent('#s-vat'), '฿0.00');
  ...
  /* แบบที่สาม: ราคาที่ตกลงกับลูกค้ารวม VAT ไว้แล้ว ต้องถอดออกก่อนลงชีท
     เพราะชีทเป็นคนบวก VAT เอง ถ้าไม่ถอด ลูกค้าจะโดนบวกซ้ำสองรอบ
     ถอดทีละบรรทัดแล้วปัดสองตำแหน่ง เหมือนที่ฝั่งชีททำ ไม่ใช่หารยอดรวมทีเดียว
       750 ÷ 1.07 = 700.93 × 1   = 700.93
       110 ÷ 1.07 = 102.80 × 20  = 2,056.00   → ยอดสินค้า 2,756.93
       ค่าส่ง 50 ÷ 1.07 = 46.73 · VAT = (2,756.93 + 46.73) × 7% = 196.26
       ยอดสุทธิ 2,999.92 — ขาด 8 สตางค์จากการปัดรายบรรทัด ซึ่งเป็นของจริง
       ต้องโชว์ตามนั้น ห้ามแต่งให้สวยเป็น 3,000 เพราะเลขที่ลงชีทคือ 2,999.92 */
  await page.fill('#items .it:first-child .i-price', '750');
  await page.selectOption('#f-vat', 'incl');
  await page.waitForTimeout(150);
  eq('ถอด VAT ออกจากยอดสินค้าทีละบรรทัด', await page.textContent('#s-sub'), '฿2,756.93');
  eq('ค่าส่งก็ถูกถอดด้วย เพราะชีทเอาค่าส่งเข้าฐานภาษี', await page.textContent('#s-ship'), '฿46.73');
  eq('VAT ที่ถอดได้', await page.textContent('#s-vat'), '฿196.26');
  eq('ยอดสุทธิกลับมาเท่าราคาที่ตกลงกับลูกค้า', await page.textContent('#s-net'), '฿2,999.92');
  truthy('ทุกแถวพิมพ์ราคาเองแล้ว บอกว่าถอดให้เรียบร้อย',
    /ถอด VAT ออกจากราคาที่พิมพ์/.test(await page.textContent('#s-vat-note')));
  eq('ใช้สีเขียว', await page.getAttribute('#s-vat-note', 'class'), 'lotline ok');

  /* เว้นช่องราคาว่าง = ใช้ราคามาตรฐานจากฐานสินค้า ซึ่งเป็นราคาก่อน VAT อยู่แล้ว
     ถอดซ้ำจะกลายเป็นขายต่ำกว่าราคาที่ร้านตั้งไว้ทุกใบ ฝั่งชีทก็ไม่ถอดเหมือนกัน */
  await page.fill('#items .it:first-child .i-price', '');
  await page.waitForTimeout(150);
  eq('ราคามาตรฐานไม่ถูกถอดซ้ำ', await page.textContent('#s-sub'), '฿2,806.00');
  /* ที่เจอของจริง: เจ้าของร้านเลือก "ราคารวม VAT แล้ว" แล้วปล่อยช่องราคาว่าง
     เพราะไม่มีอะไรบอกว่าต้องพิมพ์ตรงไหน ใบนั้นจึงไม่ได้ถอดอะไรเลย
     หน้าจอดูปกติทุกอย่าง ผิดเงียบ ๆ จนลูกค้าทัก — ต้องบอกให้เห็นตรงนั้น */
  truthy('เตือนว่ามีแถวที่ปล่อยราคาว่าง',
    /ปล่อยช่อง.*ราคาขายจริง.*ว่าง/.test(await page.textContent('#s-vat-note')));
  truthy('บอกว่าราคามาตรฐานเป็นราคาก่อน VAT อยู่แล้ว',
    /ราคาก่อน VAT อยู่แล้ว/.test(await page.textContent('#s-vat-note')));
  eq('ใช้สีเตือน', await page.getAttribute('#s-vat-note', 'class'), 'lotline warn');

  /* ยอดจากช้อปปี้ไม่มีค่าส่ง — ร้านไม่ได้เก็บเอง แพลตฟอร์มคิดของเขาต่างหาก
     ช่องค่าส่งเติมให้เองตามยอดสินค้า ถ้าไม่พิมพ์ 0 ทับ ใบนี้จะมีค่าส่งงอกขึ้นมา
     ไม่มีค่าส่งแล้วยอดสุทธิต้องกลับมาตรงกับยอดที่ลูกค้าจ่ายเป๊ะ ๆ ไม่ขาดสตางค์
       471 ÷ 1.07 = 440.19 · VAT = 440.19 × 7% = 30.81 · รวม 471.00 พอดี */
  await page.fill('#items .it:nth-child(2) .i-qty', '0');
  await page.fill('#items .it:first-child .i-price', '471');
  await page.fill('#f-ship', '0');
  await page.waitForTimeout(150);
  eq('ยอดช้อปปี้ถอด VAT แล้ว', await page.textContent('#s-sub'), '฿440.19');
  eq('ไม่มีค่าส่งก็ไม่งอกค่าส่งขึ้นมา', await page.textContent('#s-ship'), '฿0.00');
  eq('VAT ที่ถอดได้', await page.textContent('#s-vat'), '฿30.81');
  eq('ยอดสุทธิเท่ากับยอดที่ลูกค้าจ่ายพอดี', await page.textContent('#s-net'), '฿471.00');

  /* บางใบของมาร์เก็ตเพลสร้านออกค่าส่งเอง พิมพ์เองแล้วห้ามโดนล้างเป็น 0 */
  await page.fill('#f-ship', '80');
  await page.selectOption('#f-channel', 'Shopee');
  await page.waitForTimeout(120);
  eq('ค่าส่งที่พิมพ์เองไม่ถูกล้างทิ้งเพราะเปลี่ยนช่องทาง',
    await page.inputValue('#f-ship'), '80');
  await page.selectOption('#f-channel', 'หน้าร้าน');

  await page.fill('#items .it:nth-child(2) .i-qty', '20');
  await page.fill('#f-ship', '50');
  await page.fill('#items .it:first-child .i-price', '');
  await page.waitForTimeout(120);
  await page.selectOption('#f-vat', 'none');
  await page.waitForTimeout(120);
  eq('ปิด VAT แล้วยอดกลับไปเท่าเดิม', await page.textContent('#s-net'), '฿3,000.00');
  truthy('ไม่ได้ถอด VAT ก็ไม่ต้องมีข้อความนี้กวน',
    !(await page.isVisible('#s-vat-note')));
  ...
  console.log('\n   ดึงใบเสนอราคามาเป็นออเดอร์ ยอดต้องตรงกันเป๊ะ แม้มีค่าส่ง');
  /* ข้อนี้เคยยืนยันว่า "ต้องขึ้นคำเตือนว่ายอดไม่เท่ากัน ต่างกัน 3.50"
     โดยอ้างว่าใบเสนอราคาคิด VAT รวมค่าส่ง ส่วนชีทคิดเฉพาะค่าสินค้า
     ซึ่งไม่จริง — HEAD_VAT_FORMULA ของชีทรวมค่าส่งอยู่ในฐานภาษีด้วย
     ที่ต่างกันคือ recalc ของหน้าคีย์ออเดอร์ลืมค่าส่ง ไม่ใช่ชีทกับใบต่างกัน
     คำเตือนนั้นจึงเป็นอาการของบั๊ก ไม่ใช่ของที่ควรมี และข้อสอบข้อนี้
     ล็อกอาการไว้เป็นพฤติกรรมที่ถูกต้อง ทำให้บั๊กอยู่รอดมาตลอด

     พอ recalc คิดตรงกับชีทแล้ว สองยอดเท่ากันพอดี ไม่มีอะไรต้องเตือน
     1,000 (สินค้า 4×120 + 500 − ส่วนลด 30) ... ยอดสุทธิต้องเท่าที่เสนอไปเป๊ะ */
  var q2oNet = await page.evaluate(function () {
    return { net: String($('#s-net').textContent),
             err: $('#err').classList.contains('on') ? $('#err').textContent : '' };
  });
  eq('ไม่มีคำเตือนยอดไม่เท่ากันแล้ว', /ยอดไม่เท่ากัน/.test(q2oNet.err), false);
  eq('ยอดสุทธิของออเดอร์เท่ากับยอดที่เสนอไป', q2oNet.net, '฿1,070.00');
  /* ด่านนี้ยังต้องทำงานอยู่ ไม่ได้ถอดทิ้ง — ถ้าไปแก้ราคาหลังดึงใบมา ต้องเตือน */
  await page.fill('#items .it:nth-child(1) .i-price', '130');
  await page.waitForTimeout(200);
  truthy('แก้ราคาหลังดึงใบมาแล้ว ยอดขยับจากที่เสนอไปจริง',
    (await page.textContent('#s-net')) !== '฿1,070.00');
  await page.fill('#items .it:nth-child(1) .i-price', '120');
  await page.waitForTimeout(200);
  ...
  truthy('บอกตรง ๆ ว่าลงตัวไม่ได้ ไม่แกล้งบอกว่าตรงพอดี', /ทำให้ลงตัวไม่ได้/.test(sayB));
  truthy('และอธิบายว่าทำไมถึงตรงเป๊ะไม่ได้', /ปัดสองตำแหน่ง/.test(sayB));
  /* บอกแค่ "ไม่ตรง" แล้วปล่อยให้ไปนั่งลองเอง ไม่ช่วยอะไร ต้องบอกว่าทำได้แค่ไหน */
  truthy('บอกค่าที่ทำได้ใกล้ที่สุดให้เลือก', /ค่าที่ทำได้ใกล้ที่สุดคือ ฿[\d,.]+ กับ ฿[\d,.]+/.test(sayB));
  truthy('และบอกว่าตอนนี้ตั้งไว้ที่เท่าไร', /ตอนนี้ตั้งไว้ที่ ฿/.test(sayB));
  eq('ใช้สีเตือน ไม่ใช่สีเขียวว่าสำเร็จ',
    await page.getAttribute('#back-say', 'class'), 'lotline warn');

  console.log('\n   ยอด 480 ของใบช้อปปี้จริง ลงตัวพอดีอยู่แล้ว');
  /* 480 ÷ 1.07 = 448.60 · VAT 31.40 · รวม 480.00 เป๊ะ — ที่ลงตัวไม่ได้คือ 571
     ซึ่งเอาค่าส่ง 91 ที่ร้านไม่ได้รับมารวมด้วย ใช้ยอดที่ถูกตั้งแต่แรกก็ไม่มีปัญหานี้ */
  await page.fill('#f-paid', '480');
  await page.fill('#f-paid-ship', '');
  await page.click('#btn-back');
  await page.waitForTimeout(400);
  eq('ราคาต่อชิ้นของใบจริง', await page.inputValue('#items .it:first-child .i-price'), '89.72');
  eq('ยอดสุทธิตรงกับที่ลูกค้าจ่ายเป๊ะ', await page.textContent('#s-net'), '฿480.00');
  truthy('บอกว่าลงตัวพอดี',
    /ตรงกับที่ลูกค้าจ่ายพอดี/.test(await page.textContent('#back-say')));
  ...
  /* ร่างจดหมายเป็นของเจ้าของร้านเขียนเอง ไม่ได้ใส่ยอดเงินโดยตั้งใจ
     (ยอดอยู่บนใบที่แนบไปแล้ว) ที่ต้องมีคือชนิดใบ เลขที่ และท้ายจดหมายของร้าน */
  truthy('ข้อความบอกชนิดใบกับเลขที่', /ใบเสร็จรับเงิน เลขที่ ONIV/.test(draft.body));
  truthy('มีท้ายจดหมายของร้าน', /ฝ่ายขาย \(Sales Department\)/.test(draft.body));
  truthy('มีช่องทางติดต่อกลับ', /Line OA: @citiofficial/.test(draft.body));
  truthy('ไม่มีคำแทนที่ค้างอยู่', draft.body.indexOf('{') < 0);
