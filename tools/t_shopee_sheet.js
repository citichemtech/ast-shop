...
/* ============================================ สินค้าที่ยังไม่ได้ใส่ต้นทุน */

console.log('\n— สินค้าที่ยังไม่ได้ใส่ต้นทุน —');
var E = { f: FS.build({
  products: [
    { sku: 'SKU-141', name: 'มีต้นทุนแล้ว', price: 129, cost: 35 },
    { sku: 'SKU-NEW', name: 'เพิ่งเพิ่ม ยังไม่ใส่ต้นทุน', price: 200, cost: '' }
  ],
  lots: [],
  skuMap: [{ code: 'SP-141', sku: 'SKU-141', mult: 1 }, { code: 'SP-NEW', sku: 'SKU-NEW', mult: 1 }]
}) };
E.ctx = FS.load(E.f, {});
var mixed = file([
  ['2609E1', 'ที่ต้องจัดส่ง', '2026-09-05 10:00', 'ของมีต้นทุน', '3.0', 'SP-141', 1, 129, 129, 12,
   'คุณไอ', '0800000005', '9 ถ.ทดสอบ', 'Flash Express', ''],
  ['2609E2', 'ที่ต้องจัดส่ง', '2026-09-05 11:00', 'ของยังไม่ใส่ต้นทุน', 'ไซซ์เดียว', 'SP-NEW',
   2, 200, 400, 40, 'คุณเจ', '0800000006', '10 ถ.ทดสอบ', 'Flash Express', '']
]);
var pvE = E.ctx.previewShopee(mixed, {});
eq('เตือนเฉพาะตัวที่ยังไม่มีต้นทุน',
  pvE.noCost.map(function (x) { return x.sku; }), ['SKU-NEW']);
eq('บอกว่าอยู่ในกี่ใบที่จะตัด', pvE.noCost[0].orders, 1);
eq('เตือนเรื่องต้นทุนแล้วยังตัดสต๊อกได้ตามปกติ ไม่ใช่กันไว้', pvE.ready.length, 2);
eq('ตัวที่ใส่ต้นทุนแล้วไม่ถูกเตือน', pvE.noCost.length, 1);
